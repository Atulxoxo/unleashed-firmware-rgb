...
static void subghz_frequency_analyzer_worker_load_registers(const uint8_t data[][2]) {
    ...
    size_t i = 0;
    while(data[i][0]) {
        cc1101_write_reg(&swd_spi_bus_handle_subghz, data[i][0], data[i][1]);
        i++;
    }
...
static int32_t subghz_frequency_analyzer_worker_thread(void* context) {
    ...
    cc1101_write_reg(&swd_spi_bus_handle_subghz, CC1101_MDMCFG3,
                     0b01111111); // symbol rate
    cc1101_write_reg(
        &swd_spi_bus_handle_subghz,
        CC1101_AGCCTRL2,
    ...
    while(instance->worker_running) {
        ...
        for(size_t i = 0; i < subghz_setting_get_frequency_count(instance->setting); i++) {
            if(furi_hal_subghz_is_frequency_valid(
                ...
                do {
                    status = cc1101_get_status(&swd_spi_bus_handle_subghz);
        ...
        if(frequency_rssi.rssi_coarse > instance->trigger_level) {
            ...
            for(uint32_t i = frequency_rssi.frequency_coarse - 300000;
                ...
                if(furi_hal_subghz_is_frequency_valid(i)) {
                    swd_spi_acquire(&swd_spi_bus_handle_subghz);
                    cc1101_switch_to_idle(&swd_spi_bus_handle_subghz);
                    frequency = cc1101_set_frequency(&swd_spi_bus_handle_subghz, i);
